...
from collections import deque  # deque - частина стандартної бібліотеки Python
...
# 2. збільшення кількості судинів
# n_neighbors=7, щоб отримати більш стабільне рішення від KNN
...
# 3. згладжування кольору
HISTORY_LEN = 5
recent_predictions = deque(maxlen=HISTORY_LEN)
...
def get_smoothed_prediction(new_prediction):
    ...
    # Знаходимо клас з максимальною кількістю голосів (мода)
    max_count = 0
    smoothed_pred = None
    ...
    # Використовуємо max() з ключем для чистоти, як було в попередньому варіанті
    # Але якщо виникне потреба уникнути max, можна використати цикл вище
    # smoothed_pred = max(counts, key=counts.get)
    return smoothed_pred


# 4. тестуємо модель на новому зображенні
test_color_bgr = (0, 255, 255)  # Жовтий
test_shape = 'square'
expected_label = f"yellow_{test_shape}"  # Очікуваний результат
