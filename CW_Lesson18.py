...
seen_id_total = set() # лічильник об'єктів
seen_id_class = {}
...
while True:
    ...
    result = model.track(frame, conf=CONF_THRESHOLD, tracker=TRACKER, persist=True, verbose=True)
    r = result[0]
    ...
    for i in range(len(xyxy)):
        ...
        if track_id is not None:
            ...
            cv2.rectangle(frame, (x1, y1 - th - 10), (x2 + tw + 10, y2), (0, 255, 0), -1)
            cv2.putText(frame, label, (x1 + 5, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2)

            total = len(seen_id_total)
            cv2.putText(frame, f'unique objects {total}', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2)
